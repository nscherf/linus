...
export default class LinusTourController {
    ...
    create() {
        ...
        for (let id in this.tours) {
            ...
            tourLink.onclick = function () {
                this.context.startOrLoadTour(this.id, false);
            }.bind(p);
     ...
     */
    showTourEditor() {
        if (this.isShowingEditor) return;
    ...
    startOrLoadTour(name, repeat = true) {
        if (this.isShowingEditor) this.tourEditor.loadTour(this.tours[name]);
    ...
    addMarker(delay, x, y, z, text, timespan) {
        ...
        setTimeout(
            function (p) {
                p.context.addAnnotation(p.name, p.x, p.y, p.z, p.text);
            },
            this.timer,
            p
        );
        setTimeout(
            function (p) {
                p.context.removeAnnotation(p.name);
            },
    ...
    setSelection(delay, selectionMap) {
        ...
        setTimeout(
            function (p) {
                p.context.hideAllButSelected(p.value);
            },
            this.timer,
            p
        );
    ...
    clearSelection(delay) {
        ...
        setTimeout(
            function (m) {
                p.context.makeAllElementsVisible();
            },
            this.timer,
            p
        );
    ...
    moveCameraAroundY(delay, timespan) {
        ...
        setTimeout(
            function (p) {
                p.context.moveCameraAroundYHelper(p);
            },
            this.timer,
            p
        );
    ...
    moveCameraAroundYHelper(p) {
        let fps = 30;
        let angle = Math.atan(
            p.context.camera.position.x,
            p.context.camera.position.z
        );
        let radius = Math.sqrt(
            p.context.camera.position.x * p.context.camera.position.x +
                p.context.camera.position.z * p.context.camera.position.z
        );
        ...
        for (let i = 0; i < numSteps; i++) {
            ...
            setTimeout(
                function (p) {
                    p.context.camera.position.set(p.x, p.y, p.z);
                    p.context.camera.lookAt(new Vector3(0, 0, 0));
                },
                i * (1000 / fps),
                pp
            );
    ...
    /**
     * Converts a current state i (in range [0, max]) to a state with softer start/end,
     * e.g. something like [0,1,2,3,4,5] will become [0, 0.5, 1, 2.5, 4, 4.5, 5]
     */
    ease(i, max) {
        let ratio = i / max;
        let a = 2;
        let result =
            max *
            (Math.pow(ratio, a) /
                (Math.pow(ratio, a) + Math.pow(1 - ratio, a)));
    ...
    fadeParameter(delay, name, to, duration) {
        ...
        setTimeout(
            function (p) {
                p.context.fadeParameterHelper(p);
                p.context.camera.lookAt(new Vector3(0, 0, 0));
            },
            this.timer,
            p
